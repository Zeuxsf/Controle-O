...
def thread_camera():
    ...
    while rodando:
        ...
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGBA)  # DPG usa RGBA

        if not fila_frame.full():
            fila_frame.put(frame)

    captura.release()

# ── Atualiza textura com o frame atual ───────────────────────────────────────
def atualizar_frame():
    try:
        frame = fila_frame.get_nowait()
        # Normaliza pra float 0.0–1.0 que o DPG espera
        dados = (frame.astype(np.float32) / 255.0).flatten().tolist()
...
def main():
    ...
    t = threading.Thread(target=thread_camera, daemon=True)
    t.start()
